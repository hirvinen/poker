...
const sameValue = (a,b) => (a.value === b.value || (a.value * b.value === 0))
const sameSuit = (a,b) => (a.suit.short === b.suit.short || (a.value * b.value === 0))
const cardEquals = (a,b) => (sameValue(a,b) && sameSuit(a,b))
...
const sameValueCards = (deck, card) =>
  deck.filter( c => sameValue(c, card))
...
const countValues = (hand) => {
  const valueCounts = Array.from({length: 14}, () => 0)
  hand.forEach(card => {
...
const hasStraight = (hand) => {
  ...
  if (!hasJoker(hand)) {
    ...
    if (lowest.value == 1) {
      const [ace14, secondLowest, ...rest] = sortedHand
      ace14.value = 14
...
const hasPair10ToAce = (hand) => {
  ...
  if (hasJoker(hand)) {
    const [highest] = sortedHand.slice(-1)
    return highest.value >= 10
  }
  return hasNOfAKind(hand.filter(card => card.value >= 10), 2)
}
