...
const hasRoyalFlush = (hand) => {
  ...
  const [lowest, secondLowest] = hand
  const [highest] = hand.slice(-1)
  if (lowest.value === 0 && 
...
const hasStraight = (hand) => {
  ...
  if (!hasJoker(hand)) {
    ...
    if (lowest.value == 1) {
      const [ace, secondLowest, ...rest] = hand
...
const classify = (hand = []) => {
  ...
  if (hasNOfAKind(sortedHand, 5))   { return 'fiveOfAKind' }
  if (hasRoyalFlush(sortedHand))    { return 'royalFlush' }
  if (hasStraightFlush(sortedHand)) { return 'straightFlush' }
  if (hasNOfAKind(sortedHand, 4))   { return 'fourOfAKind' }
  if (hasFullHouse(sortedHand))     { return 'fullHouse' }
  if (hasFlush(sortedHand))         { return 'flush' }
  if (hasStraight(sortedHand))      { return 'straight' }
  if (hasNOfAKind(sortedHand, 3))   { return 'threeOfAKind' }
  if (hasTwoPairs(sortedHand))      { return 'twoPairs' }
  if (hasPair10ToAce(sortedHand))   { return 'pair10ToAce' }
  return 'none'
}
