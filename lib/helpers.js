...
const getUniqueIntArray = (count = 0, max) => {
  const ints = []
  const limit = max ? max + 1 : count + 1
  while (ints.length < count) {
    let candidate = Math.floor(Math.random() * limit)
    if (!ints.includes(candidate)) ints.push(candidate)
  }
}
